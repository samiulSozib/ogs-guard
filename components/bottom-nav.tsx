...
import { motion, AnimatePresence } from "framer-motion"
import { cn } from "@/lib/utils"
import { guardBottomNavItems, clientBottomNavItems } from "./bottom-nav-icon"

interface BottomNavProps {
  className?: string
  variant?: "default" | "glass" | "neumorphic"
  userType?: "guard" | "client" | null
}

export function BottomNav({ className, variant = "glass", userType: propUserType }: BottomNavProps) {
  const pathname = usePathname()
  const [userType, setUserType] = React.useState<"guard" | "client" | null>(null)
  const [mounted, setMounted] = React.useState(false)

  React.useEffect(() => {
    setMounted(true)
    // Get user type from props or localStorage
    const type = propUserType || localStorage.getItem('user_type') as "guard" | "client" | null
    setUserType(type)
  }, [propUserType])
  ...
  const bottomNavItems = userType === 'client' ? clientBottomNavItems : guardBottomNavItems
